...
class SurfaceProcessor:
    def __init__(self, config_path="input_params.json", json_path="outputs.json/key_archivos.json", radi=None, threshold_file="outputs.vfinder/key_single_vacancy.json"):
...
class ClusterDumpProcessor:
    def __init__(self, file_path: str, decimals: int = 5):
        ...
        self.header = None
        self.subset = None
        self.divisions_of_cluster = self.config[0]['divisions_of_cluster']
...
class ClusterProcessor:
    def __init__(self,defect):
        ...
        self.radio_sonda = self.config['radius']
        self.smoothing_leveled = self.config['smoothing level']
        self.cutoff_radius = self.config['cutoff radius']
...
if __name__ == "__main__":
    ...
    print(f"Defectos: {defect_file_list}")
    relax = config['relax']
    radius_training = config['radius_training']
    radius = config['radius']
    smoothing_level_training = ['smoothing_level_training']
    other_method = config['other method']
    
    processor = ClusterProcessor(defect_file_list)
    processor.run()
