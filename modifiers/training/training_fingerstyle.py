...
def read_dump_and_translate_to_com(dump_path):
    ...
    # Extraer x, y, z de cada línea hasta la siguiente sección
    ...
    # Calcular centro de masa (promedio de cada eje)
    com = tuple(coords_originales.mean(axis=0))  # (com_x, com_y, com_z)

    # Trasladar las coordenadas restando el COM
...
def export_features_to_csv(dump_paths, output_csv):
    """
    Para cada archivo .dump en dump_paths:
      1) Lee y traslada coordenadas al COM.
      2) Calcula normas y ordena de menor a mayor.
      3) Calcula un conjunto amplio de estadísticas sobre las normas.
      4) Escribe una fila en el CSV con:
...
if __name__ == "__main__":
    ...
    export_features_to_csv(dump_files, output_csv_path)
